...
namespace ActivitySchedulerUnitTests
{
	...
	public class CamperRequestsTests
	{
		...
		[Test]
		public void GenerateCamperActivityPreferences_ValidInput_GeneratesExpectedList()
		{
			// Arrange load the known good camper list
			List<CamperRequests> camperRequestsList = CamperRequests.ReadCamperRequests(GoodFileLocation,
				ActivityDefinitionTests.DefaultActivityDefinitions, _logger);

			// Act - Generate camper preferences
			Dictionary<Camper, List<ActivityDefinition>> camperPreferences = 
				CamperRequests.GenerateCamperActivityPreferences(camperRequestsList);

			// Assert - compare the camper preferences to the camper requests
			foreach (CamperRequests camperRequests in camperRequestsList)
			{
				Assert.That(camperPreferences, Contains.Key(camperRequests.Camper),
					$"Preferences");
				Assert.That(camperPreferences[camperRequests.Camper], 
					Has.Count.EqualTo(camperRequests.ActivityRequests.Count + 1),
					$"Camper preferences for {camperRequests.Camper.FullName}");
				foreach (ActivityRequest activityRequest in camperRequests.ActivityRequests)
				{
					// Ranks are 1 based but list is 0 based
					Assert.That(camperPreferences[camperRequests.Camper][activityRequest.Rank-1].Name,
						Is.EqualTo(activityRequest.Activity.Name),
						$"{camperRequests.Camper.FullName} preference at index {activityRequest.Rank}");
				}
				Assert.That(camperPreferences[camperRequests.Camper][camperRequests.ActivityRequests.Count].Name,
					Is.EqualTo(camperRequests.AlternateActivity.Name),
					$"{camperRequests.Camper.FullName} preference for alternate activity");
